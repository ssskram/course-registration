...
module.exports = async (context, timer) => {
  ...
  const courses = await fetch(
    "https://365proxy.azurewebsites.us/iphelp/allCourses",
    {
      method: "GET",
      headers: new Headers({
  ...
  const registrations = await fetch(
    "https://365proxy.azurewebsites.us/iphelp/allCourseRegistrations",
    {
      method: "GET",
      headers: new Headers({
        Authorization: "Bearer " + process.env.APP_365_API
  ...
  courses.forEach(course => {
    ...
    const allEnrollments = registrations.filter(
      reg => reg.courseCode == course.courseCode
    );
    const activeEnrollments = allEnrollments.filter(
      reg => reg.registrationStatus == "Active"
    );
    ...
    if (activeEnrollments.length < maxCapacity) {
      if (waitlistedEnrollments.length > 0) {
        ...
        const sortedByDateSubmitted = waitlistedEnrollments.sort(
          (a, b) => new Date(a.startDate) - new Date(b.startDate)
        );
        const registrationsToBump = sortedByDateSubmitted.slice(0, spotsToFill);
        registrationsToBump.forEach(registration => {
          fetch(
            "https://365proxy.azurewebsites.us/iphelp/updateCourseRegistration?id=" +
  ...
  impendingCourses.forEach(course => {
    ...
    courseRegistrations
      .filter(c => c.registrationStatus == "Active" && c.hasEmail == true)
      .forEach(activeReg => {
        const cancelUrl =
          process.env.CANCEL_URL +
          "&id=" +
          activeReg.registrationId +
          "&user=" +
          activeReg.user;
